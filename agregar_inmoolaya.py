...
# -*- coding: utf-8 -*-
"""
agregar_inmoolaya.py -- Corre SOLO el scraper de Inmo Olaya (unos 230
fichas, unos minutos) y mete lo que encuentre en hoteles_cache.json, sin
tener que lanzar el scraper.py completo (que tarda 20-40 min y repasa 20
portales).

Es un empujon puntual para verlo ya en la web hoy mismo. A partir de manana,
como la llamada a scrape_inmoolaya() ya esta activada dentro de scraper.py,
la pasada diaria normal se encarga sola de mantenerlo actualizado (nuevos
anuncios, precios que cambian, etc.) -- este script puntual ya no hara
falta usarlo mas.

Uso (desde C:\\HotelMonitor):
    python agregar_inmoolaya.py
...
def main():
    ...
    print(f'\n{nuevos} anuncios nuevos de Inmo Olaya anadidos al cache.')
